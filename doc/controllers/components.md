...
```php
function archiveComponent(int $productFamilyId, string $componentId): ?ComponentResponse
```
...
```php
function updateDefaultPricePointForComponent(int $componentId, int $pricePointId): void
```
...
$componentsController->updateDefaultPricePointForComponent(
    $componentId,
    $pricePointId
);
```
...
```php
function updateComponent(string $componentId, ?UpdateComponentRequest $body = null): void
```
