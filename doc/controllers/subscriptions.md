...
```php
function overrideSubscription(string $subscriptionId, ?OverrideSubscriptionRequest $body = null): void
```
...
```php
function purgeSubscription(string $subscriptionId, int $ack, ?array $cascade = null): void
```
...
```php
function readSubscription(string $subscriptionId, ?array $mInclude = null): ?SubscriptionResponse
```
...
```php
function deleteCouponFromSubscription(string $subscriptionId, ?string $couponCode = null): ?string
```
...
| `state` | [`?string(SubscriptionState)`](../../doc/models/subscription-state.md) | Query, Optional | The current state of the subscription |
